/*Date Functions*/
/proc/FindDate(Date as text)
	if(!Date) return null
	var/list/Dates = splittext(Date,"/")
	var/Day		= Dates[1]
	var/Month	= Dates[2]
	var/Year	= Dates[3]
...
/proc/AddDays(Date, Days, Months, Years)
...
/proc/SubtractDays(Date, Days, Months, Years)
	if(istext(Date)) Date = FindDate(Date)
	else if(istype(Date,/list))
	else return
	var/StartDay	= text2num(Date[1])
	var/StartMonth	= text2num(Date[2]) - Months
	var/StartYear	= text2num(Date[3]) - Years
	StartDay -= Days
	while(StartMonth <= 0)
		StartMonth += 12
		StartYear--
...
/proc/TextMonth2NumMonth(incoming)
	switch(incoming)
		if("Jan") return 1
		if("Feb") return 2
		if("Mar") return 3
...
/proc/NumMonth2TextMonth(incoming)
	switch(incoming)
		if(1) return "January"
...
/proc/Days_Difference(DateOne , DateTwo)
	var/ListOne = ValidateDate(DateOne)
	var/ListTwo = ValidateDate(DateTwo)
	...
	var/TotalOne = DaysTotal(DateOne[1],DateOne[2],DateOne[3])
	var/TotalTwo = DaysTotal(DateTwo[1],DateTwo[2],DateTwo[3])
