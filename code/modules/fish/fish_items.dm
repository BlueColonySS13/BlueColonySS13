...
/obj/item/fish_net/suicide_act(mob/user)
	...
	viewers(user) << pick("<span class='danger'>\The [user] places the on top of [TU.his] head, [TU.his] fingers tangled in the netting! It looks like [TU.hes] trying to commit suicide.</span>")
	return (OXYLOSS)
...
/obj/item/tank_brush/suicide_act(mob/user)
	var/datum/gender/TU = gender_datums[user.get_visible_gender()]
	to_chat(viewers(user), "<span class='danger'>[user] vigorously scrubbing [TU.him] self raw with the [name]! It looks like  [TU.is] trying to commit suicide!</span>")
...
/obj/item/fish/catfish/attackby(var/obj/item/O, var/mob/user as mob)
	if(is_sharp(O))
		...
		new /obj/item/weapon/reagent_containers/food/snacks/sushi/catfishmeat(get_turf(src))
		new /obj/item/weapon/reagent_containers/food/snacks/sushi/catfishmeat(get_turf(src))
		qdel(src)
		return
	..()
...
/obj/item/fish/salmon/attackby(var/obj/item/O, var/mob/user as mob)
	if(is_sharp(O))
		to_chat(user, "You carefully clean and gut \the [src.name].")
		new /obj/item/weapon/reagent_containers/food/snacks/sushi/salmonmeat(get_turf(src))
		new /obj/item/weapon/reagent_containers/food/snacks/sushi/salmonmeat(get_turf(src))
		qdel(src)
		return
	..()
