...
/obj/machinery/fishtank/attack_hand(mob/living/simple_mob/M)
	if(istype(M, /mob/living/simple_mob/animal/passive/cat))
...
//						if(istype(M, /mob/living/simple_animal/cat/Syndi))
//							do_sparks(3, 1, src)
			...
			return ..()
	else if(istype(M, /mob/living/simple_mob/animal/space/bear))
...
*/
/obj/machinery/fishtank/proc/deconstruct(disassembled = TRUE)
	if(QDELETED(src))
		return
	if(!disassembled)
		...
		new /obj/item/stack/material/glass(get_turf(src), shard_count + 1)		//Produce the appropriate number of glass sheets, in a single stack
	qdel(src)
