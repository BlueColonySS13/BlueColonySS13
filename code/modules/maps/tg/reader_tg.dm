 ...
 */
/dmm_suite/proc/load_map_tg(dmm_file as file, x_offset as num, y_offset as num, z_offset as num, cropMap as num, measureOnly as num)
						...
						if(ycrd <= world.maxy && ycrd >= 1)
							...
							for(var/tpos = 1 to length(line) - key_len + 1 step key_len)
								...
								if(xcrd >= 1)
									...
									parse_grid_tg(grid_models[model_key], xcrd, ycrd, zcrd, LM)
 ...
 *
 */
/dmm_suite/proc/parse_grid_tg(model as text,xcrd as num,ycrd as num,zcrd as num, dmm_suite/loaded_map/LM)
	/*Method parse_grid()
	...
	*/
	...
	if(cached)
		...
		do
			...
			dpos = find_next_delimiter_position_tg(model, old_position, ",", "{", "}") //find next delimiter (comma here) that's not within {...}
	...
	if(members[index] != /area/template_noop)
		...
		var/atom/instance
		_preloader.tg_setup(members_attributes[index])//preloader for assigning  set variables on atom creation
	...
	if(members[first_turf_index] != /turf/template_noop)
		T = instance_atom_tg(members[first_turf_index],members_attributes[first_turf_index],xcrd,ycrd,zcrd)
	...
	if(T)
		...
		while(index <= members.len - 1) // Last item is an /area
			...
			T = instance_atom_tg(members[index],members_attributes[index],xcrd,ycrd,zcrd)//instance new turf
	...
	for(index in 1 to first_turf_index-1)
		instance_atom_tg(members[index],members_attributes[index],xcrd,ycrd,zcrd)
		CHECK_TICK
...
/dmm_suite/proc/instance_atom_tg(path,list/attributes, x, y, z)
	var/atom/instance
	_preloader.tg_setup(attributes, path)
...
/dmm_suite/proc/find_next_delimiter_position_tg(text as text,initial_position as num, delimiter=",",opening_escape=quote,closing_escape=quote)
...
/dmm_suite/proc/readlist_tg(text as text, delimiter=",")
	...
	do
		...
		position = find_next_delimiter_position_tg(text,old_position,delimiter)
		...
		if(equal_position)//associative var, so do the association
			...
			//Check for list
				...
				trim_right = readlist_tg(copytext(trim_right,6,length(trim_right)))
...
/dmm_suite/preloader/proc/tg_setup(list/the_attributes, path)
	if(the_attributes.len)
		use_preloader = TRUE
		attributes = the_attributes
		target_path = path

/dmm_suite/preloader/proc/tg_load(atom/what)
	if(istype(what, /area))
		return
	for(var/attribute in attributes)
		...
		message_admins("Attributes are: [what.vars[attribute]].", 1)
