	...
	nS_Keyword
		...
		kwDef
			Parse(n_Parser/nS_Parser/parser)
				...
				if(!parser.CheckToken("(", /token/symbol))
					return KW_FAIL
				for(var/i in 1 to 100) //This is terrible but this entire folder needs a bigger rewrite
					//for now parameters can be separated by whitespace - they don't need a comma in between
