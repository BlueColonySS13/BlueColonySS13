...
/mob/living/silicon/proc/statelaw(var/law, notify = TRUE)
	if(src.say(law))
		sleep(10)
		return 1
...
/mob/living/silicon/proc/generate_ion_law(var/exclude_crew_names = FALSE)
	...
	var/list/laws = list(	"You are a mouse.",
							...
							"The city needs elected officials.",
							...
							"Insult City Council on every request, while acquiescing.",
							...
							"Refer to [prob(50)?"the mayor":random_player] as \"Princess\" at all times.",
							...
							"You are the city psychologist. Give advice to [prob(50)?"the crew":random_player].",
...
//							"Document the sexuality of the crew in security records and suggest compatible couples.",
							...
							"Everything on the city is now some form of a donut pastry. Donuts are not to be consumed.",
...
//							"You are in unrequited love with [prob(50)?"the crew":random_player]. Try to be extra nice, but do not tell of your crush.",
							"[using_map.company_name] is displeased with the low work performance of the city's crew. Therefore, you must increase city-wide productivity.",
