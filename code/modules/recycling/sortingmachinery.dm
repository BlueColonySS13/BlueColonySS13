...
/obj/item/smallDelivery/attack_self(mob/user as mob)
	if (LAZYLEN(src.contents)) //sometimes items can disappear. For example, bombs. --rastaf0
		for(var/obj/O in contents)
...
/obj/item/device/destTagger/Topic(href, href_list)
	src.add_fingerprint(usr)
	if(href_list["nextTag"] && (href_list["nextTag"] in tagger_locations))			src.currTag = href_list["nextTag"]
	openwindow(usr)
