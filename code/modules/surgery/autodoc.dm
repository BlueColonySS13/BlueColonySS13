...
/datum/autodoc
	...
	var/money_department = DEPT_HEALTHCARE
...
/datum/autodoc/proc/scan_user()
	...
	for(var/obj/item/organ/external/external in patient.bad_external_organs)
		...
		if(AUTODOC_DAMAGE in possible_operations)
			if(external.brute_dam || external.burn_dam)
				if(external.robotic == ORGAN_ROBOT)
					continue
				patchnote.surgery_operations |= AUTODOC_DAMAGE
	...
	to_chat(patient, SPAN_NOTICE("\The autodoc signals that the scan process is complete."))
...
/datum/autodoc/capitalist_autodoc/New()
	. = ..()
	linked_account = dept_acc_by_id(money_department)
...
/datum/autodoc/capitalist_autodoc/proc/charge(var/amount = 100)
	...
	charge_to_account(patient_account.account_number, linked_account.owner_name, "Autodoc surgery", "[src]", -amount)

	charge_to_account(linked_account.account_number, patient_account.owner_name, "Autodoc surgery", "[src]", amount)
...
/datum/autodoc/capitalist_autodoc/proc/login(mob/user)
	if(!patient)
		patient_account = null
		return
