...
/client/proc/startSinglo()
	...
	set desc = "Sets up the singularity and all machines to get power flowing through the city"
	...
	if(istype(planet))
		...
		if(!isnull(new_hour))
			...
			if(!isnull(new_minute))
				var/datum/time/new_time = current_time_datum
				new_time.add_hours(new_hour)
				new_time.add_minutes(new_minute)
				spawn(1)
					planet.update_sun()
