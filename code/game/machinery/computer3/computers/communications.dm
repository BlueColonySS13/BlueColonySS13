	...
	if("announce" in href_list)
		if(authenticated==2)
			if(message_cooldown)
				...
				return
			var/input = input(usr, "Please write a message to announce to the city residents.", "Priority Announcement")
...
/datum/file/program/communications/interact()
	...
	switch(state)
		...
		if(STATE_STATUSDISPLAY)
			...
			dat += "\[ <A HREF='?src=\ref[src];setstat;statdisp=time'>City Time</A> \]"
