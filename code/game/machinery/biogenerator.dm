		...
		switch(menustat)
			if("menu")
				if(beaker)
					...
					dat += "<A href='?src=\ref[src];action=create;item=meat;cost=50'>Slab of Synthetic Meat</A> <FONT COLOR=blue>([round(50/build_eff)])</FONT> | <A href='?src=\ref[src];action=create;item=meat5;cost=24s0'>x5</A><BR>"
					dat += "Nutrient:<BR>"
