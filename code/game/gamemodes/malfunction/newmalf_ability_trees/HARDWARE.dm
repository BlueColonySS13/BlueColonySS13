...
/datum/game_mode/malfunction/verb/ai_destroy_station()
	set category = "Hardware"
	set name = "Destroy City"
	set desc = "Activates or deactivates self destruct sequence of this City. Sequence takes two minutes, and if you are shut down before timer reaches zero it will be cancelled."
	var/mob/living/silicon/ai/user = usr
	...
	var/choice = alert("Really destroy City?", "City self-destruct", "YES", "NO")
	if(choice != "YES")
		return
	if(!ability_prechecks(user, 0, 0))
		return
	user << "***** CITY SELF-DESTRUCT SEQUENCE INITIATED *****"
